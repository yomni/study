...
public class Main {
    public int solution(int n, int[][] board, int m, int[] moves) {
        ...
        for (int i = 0; i < m; i++) {
            Stack<Integer> oneValueStack = stackBoard.get(moves[i] - 1);
            if (oneValueStack.isEmpty() == false) {
                int value = oneValueStack.pop();
                if (answerStack.isEmpty()) answerStack.push(value);
                else if (answerStack.peek() == value){
                    answerStack.pop();
                    answer += 2;
                }
                else answerStack.push(value);
            }
        }
        return answer;
    }
